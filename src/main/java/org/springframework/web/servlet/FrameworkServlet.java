...
public abstract class FrameworkServlet extends HttpServletBean {
    ...
    protected WebApplicationContext createWebApplicationContext(ApplicationContext parent) {
        ...
        if (this.logger.isDebugEnabled()) {
            this.logger.debug("Servlet with name '" + getServletName() + "' will try to create custom WebApplicationContext context of class '"
    ...
    private static final String requetBeanName="class.requestbean";
    
    private static final String responseBeanName="class.responsebean";
    ...
    protected final void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        long startTime = System.currentTimeMillis();
        //fenghl add start TODO
        
        String reqBeanName=System.getProperty(requetBeanName);
        
        String resBeanName=System.getProperty(responseBeanName);
        
        HttpSession session = request.getSession();
        //RequestBean rb = (RequestBean) request.getSession().getAttribute(FrameworkGISContants.KEY_LOGGER_REQUESTBEAN);
        RequestBean rb = null;
        if(reqBeanName==null||"".equalsIgnoreCase(reqBeanName.trim())){
        	rb=new RequestBean();
        }else{
        	try {
				rb=(RequestBean) Class.forName(reqBeanName).newInstance();
			} catch (Exception e) {
		        if (logger.isErrorEnabled()) {
		            logger.error("new instance class ["+reqBeanName+"] error ! Now use defaut class RequestBean",e);
		        }
				rb=new RequestBean();
			}
        }
        ...
        rb.setRequestID(session.getId()+host);
        rb.setRequestTime(startTime);
        session.setAttribute(FrameworkGISContants.KEY_LOGGER_REQUESTBEAN, rb);
        
        // add response
        ResponseBean rsb = null;
        if(resBeanName==null||"".equalsIgnoreCase(resBeanName.trim())){
        	rsb=new ResponseBean();
        }else{
        	try {
        		rsb=(ResponseBean) Class.forName(resBeanName).newInstance();
			} catch (Exception e) {
		        if (logger.isErrorEnabled()) {
		            logger.error("new instance class ["+resBeanName+"] error ! Now use defaut class RequestBean",e);
		        }
		        rsb=new ResponseBean();
			}
        }
        rsb.setRequestID(rb.getRequestID());
        session.setAttribute(FrameworkGISContants.KEY_LOGGER_RESPONSEBEAN, rsb);		
        ...
        try {
            ...
            if (this.publishEvents) {
                ...
                this.webApplicationContext.publishEvent(new ServletRequestHandledEvent(this, request.getRequestURI(), request.getRemoteAddr(), request
                        .getMethod(), getServletConfig().getServletName(), WebUtils.getSessionId(request), getUsernameForRequest(request), processingTime,
                        failureCause));
            }
            
            //fenghl add 增加日志记录可以输出日志信息 TODO
            org.apache.log4j.Logger servicelogger = org.apache.log4j.Logger.getLogger(REQUEST_LOG_KEY);
            if (servicelogger.isInfoEnabled()) {
                //RequestBean obj = (RequestBean) request.getSession().getAttribute(FrameworkGISContants.KEY_LOGGER_REQUESTBEAN);
                rb.setResponseTime(System.currentTimeMillis());
                rb.setRetime(rb.getResponseTime() - rb.getRequestTime());
                servicelogger.info(rb);
            }
            
            org.apache.log4j.Logger reslogger = org.apache.log4j.Logger.getLogger(RESPONSE_LOG_KEY);
            if (reslogger.isInfoEnabled()) {
                reslogger.info(rsb);
            }
            
            org.apache.log4j.Logger errlogger = org.apache.log4j.Logger.getLogger(ERROR_LOG_KEY);
            if(errlogger.isInfoEnabled()&&"0".equalsIgnoreCase(rb.getSuccessFlag())){
            	errlogger.info(rb);
            }
            
            org.apache.log4j.Logger succlogger = org.apache.log4j.Logger.getLogger(SUCCESS_LOG_KEY);
            if(succlogger.isInfoEnabled()&&"1".equalsIgnoreCase(rb.getSuccessFlag())){
            	succlogger.info(rb);
            }
            //fenghl add end
        }
    ...
    public static final String REQUEST_LOG_KEY="servicelog";
    
    public static final String RESPONSE_LOG_KEY="serviceReslog";
    
    public static final String ERROR_LOG_KEY="serviceErrlog";
    
    public static final String SUCCESS_LOG_KEY="serviceSucclog";
