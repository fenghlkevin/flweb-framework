...
public class MappingJacksonJsonView extends AbstractView {
    ...
    protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            bufferedOS = new BufferedOutputStream(byteo, 10240);
            @SuppressWarnings("unchecked")
            Map<String, Object> str = (Map<String, Object>) value;
            Object temp = str.get(FrameworkGISContants.SERVICE_KEY_RESPONSE);

            /**
             * 当结果为string时候,则判断callback并拼接
             */
            if (temp instanceof String) {
                StringBuffer reValue = new StringBuffer();
                String tempStr = (String) temp;
                if (callback != null && !"".equalsIgnoreCase(callback)) {
                    if (!tempStr.startsWith(callback)) {
                        reValue.append(callback).append("(").append(tempStr);
                    ...
                    reValue.append(tempStr);
                }
                byteo.write(reValue.toString().getBytes("UTF-8"));
            }
            else {
                if (callback != null && !"".equalsIgnoreCase(callback)) {
                    byteo.write((callback + "(").getBytes());
                }

                JsonGenerator generator = objectMapper.getJsonFactory().createJsonGenerator(bufferedOS, encoding);
                if (prefixJson) {
                    generator.writeRaw("{} && ");
                }
                ...
                if (callback != null && !"".equalsIgnoreCase(callback)) {
                    String args = request.getParameter("args");
                    if (args != null && !"".equalsIgnoreCase(args)) {
                        byteo.write((",").getBytes());
                        byteo.write(("\"").getBytes());
                        byteo.write((args).getBytes());
                        byteo.write(("\"").getBytes());
                    }
            ...
            if (callback != null && !"".equalsIgnoreCase(callback)) {
                ...
                response.getOutputStream().write(newStr.getBytes());
                response.getOutputStream().flush();
            }

            request.setAttribute(FrameworkGISContants.CONTANT_RESONSE_ATTRIBUTE, bs);
        }
        finally {
            if (bufferedOS != null) {
                bufferedOS.close();
