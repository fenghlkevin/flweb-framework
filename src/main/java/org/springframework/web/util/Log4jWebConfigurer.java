...
public abstract class Log4jWebConfigurer {
    ...
    private static final String CONFIG_LOCATION_DEFAULTNAME="log4j.xml";
    ...
    public static void initLogging(ServletContext servletContext) {
        ...
        //MAKESOURCE fenghl add
        
        String location=null;
        
        if (servletContext != null) {
            location = servletContext.getInitParameter(CONFIG_LOCATION_PARAM);
        }
        /**
         * 如果从servlet中读取配置失败，转为从System.Property中读取
         */
        if (!pathRight(location)) {
         ...
         */
        if (!pathRight(location) && servletContext != null) {
            location = servletContext.getRealPath("/") + File.separator + "WEB-INF" + File.separator + "conf" + File.separator
                    + Log4jWebConfigurer.CONFIG_LOCATION_DEFAULTNAME;
        }
        if (!pathRight(location)) {
            throw new NServiceInternalException("未合适的log4j配置文件");
        }
        
        
        
        //String location = System.getProperty(CONFIG_LOCATION_PARAM);
        //MAKESOURCE fenghl add
        if (location != null) {
...
//                }
//                else 
                	if (isFileUrl(location)) {
                    String templocation=location.replaceAll("system:", "");
                 // Resolve system property placeholders before resolving real path.
                    location = SystemPropertyUtils.resolvePlaceholders(location);
                    location = WebUtils.getRealPath(servletContext, location);
                }
                ...
                //MAKESOURCE fenghl add
                String intervalString = System.getProperty(REFRESH_INTERVAL_PARAM);
                if (intervalString != null && !"".equalsIgnoreCase(intervalString.trim())) {
                    //MAKESOURCE fenghl add
    ...
    private static boolean pathRight(String path) {
    ...
    public static boolean isFileUrl(String resourceLocation) {
        if (resourceLocation == null) {
            return false;
        }
        if (resourceLocation.startsWith("system:")) {
            return true;
        }
        try {
