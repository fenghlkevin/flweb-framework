...
public class ContextLoader {
    ...
    protected WebApplicationContext createWebApplicationContext(ServletContext sc, ApplicationContext parent) {
        ...
        if (sc != null) {
            path = sc.getInitParameter(CONFIG_LOCATION_PARAM);
        }
        /**
         * 如果从servlet中读取配置失败，转为从System.Property中读取
         */
        if (!pathRight(path)) {
            path = System.getProperty(CONFIG_LOCATION_PARAM);
        }

        /**
         * 如果系统中也不存在配置，则取得WEB-INF 环境变量下的RootConfig
         */
        if (!pathRight(path) && sc != null) {
            path = "file:"+sc.getRealPath("/") + File.separator + "WEB-INF" + File.separator + "conf" + File.separator
                    + "+*.xml";
        }
        if (!pathRight(path)) {
            throw new NServiceInternalException("未合适的contextConfigLocation");
        }
        
        wac.setConfigLocation(path);
        
//        wac.setConfigLocation(System.getProperty(CONFIG_LOCATION_PARAM));
