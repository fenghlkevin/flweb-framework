...
public abstract class PropertiesLoaderSupport {
    ...
    private static final String PARAM_SYSTEMLOCATION = "systemLocation";

    public void setSystemLocation(String nullvalue) {
        String path = System.getProperty(PARAM_SYSTEMLOCATION);
        if (path == null || "".equalsIgnoreCase(path.trim())) {
            System.err.println("[path] 为空，不加载 RootConfig--[systemLocation]");
        }else{
        	String[] paths = path.split(",");
            List<Resource> locationslist = new ArrayList<Resource>();
            for (String onepath : paths) {
                if ("".equalsIgnoreCase(onepath.trim())) {
                    continue;
                }
                File f = new File(onepath);
                if (!f.exists() || !f.isFile()) {
                    System.err.println("[path = " + onepath + "] 路径不存在或不是文件，不加载springLocation");
                }
                FileSystemResource fresource = new FileSystemResource(f);
                locationslist.add(fresource);
            }
            this.locations = locationslist.toArray(new Resource[locationslist.size()]);
        }
    }
