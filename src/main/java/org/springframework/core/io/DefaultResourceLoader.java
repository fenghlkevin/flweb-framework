...
public class DefaultResourceLoader implements ResourceLoader {
	...
	private static final String ABSFILE_URL_PREFIX = "absolutepath:";
	public Resource getResource(String location) {
		Assert.notNull(location, "Location must not be null");
		if (location.startsWith(CLASSPATH_URL_PREFIX)) {
			return new ClassPathResource(location.substring(CLASSPATH_URL_PREFIX.length()), getClassLoader());
		}else if(location.startsWith(ABSFILE_URL_PREFIX)){
			String temp = location+File.separator;
			temp = temp.substring(temp.indexOf(ABSFILE_URL_PREFIX)+ABSFILE_URL_PREFIX.length());
			...
			if (temp.indexOf("${") >= 0) {
				for (String key : keys) {
					...
					temp = temp.replace("${" + key + "}", tempValue);
				}
			}
